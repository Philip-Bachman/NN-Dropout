classdef LDNet < handle
    % An LDNet is a composition of LDLayers.
    %
    ...
    methods
        ...
        function [ A_post A_pre ] = feedforward(self, X, M, Ws)
            ...
            for i=1:self.layer_count,
                ...
                if (i == 1)
                    ...
                    A_in = LDNet.bias(A_post{i-1});
                end
                Mi = M{i};
                drop_rates = 1 - (sum(Mi,2) ./ size(Mi,2));
                drop_rates(drop_rates > 1e-5) = self.drop_hidden;
                drop_scales = ones(size(drop_rates));
                if (i > 1)
                    %drop_scales = (1 ./ (1 - drop_rates));
                    drop_scales(drop_rates < 1e-5) = 0.5;
                end
                [post pre] = lay_i.feedforward(...
                    (A_in .* Mi), Ws(i).W, drop_scales);
                A_post{i} = post;
                if (nargout > 1)
                    A_pre{i} = pre;
                end
        ...
        function [ dLdWs dLdX ] = backprop(self, ...
            ...
            for i=self.layer_count:-1:1,
                lay_i = self.layers{i};
                if (i == self.layer_count)
                    M_post = ones(size(A_post{i}));
                else
                    M_post = M{i+1};
                    M_post = M_post(:,1:(end-1));
                end
                M_pre = M{i};
                drop_rates = 1 - (sum(M_pre,2) ./ size(M_pre,2));
                drop_rates(drop_rates > 1e-5) = self.drop_hidden;
                drop_scales = ones(size(drop_rates));
                if (i > 1)
                    %drop_scales = (1 ./ (1 - drop_rates));
                ...
                [dLdWi dLdAi] = lay_i.backprop((dLdA_post{i} .* M_post), ...
                    dLdA_pre{i}, (A_post{i}.*M_post), (A_in.*M_pre), Ws(i).W, drop_scales);
                if (i == 1)
                    dLdX = dLdAi(:,1:(end-1));
                else
...
%                     drawnow();
        ...
        function [ L dLdWs ] = dev_loss_W(self, Ws, X, Y, M, b_size, d_reps)
            ...
            % Add activation regularization loss/gradient
            L_reg = 0;
            for i=self.layer_count,
                if (i == self.layer_count)
                    bias = 0;
                else
                    bias = 0.1;
                end
                L_reg = L_reg + ((self.lam_l2a / b_size) * sum(sum((A_post{i}-bias).^2)));
                dLdA_post{i} = dLdA_post{i} + ((2*(self.lam_l2a/b_size)) * (A_post{i}-bias));
            end 
