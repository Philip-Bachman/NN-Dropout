classdef BiliLayer < handle
    ...
    methods
        function [ self ] = BiliLayer(in_dims, out_dim, afun)
            self.act_trans = afun;
        ...
        function [ Ws ] = init_weights(self, wt_scale, b_scale)
            ...
            for i=1:self.dim_output,
                Ws(i).W = wt_scale * randn(l_dim,r_dim);
                Ws(i).w = wt_scale * randn((l_dim+r_dim),1);
        ...
        function [ Wv ] = vector_weights(self, Ws)
            ...
            % which is assumed to come from this BiliLayer instance.
            %
        ...
        function [ dLdW dLdX1 dLdX2 ] = backprop(self, dLdA, A, X1, X2, Wv)
            ...
            for i=1:size(X1,1),
                ...
                % Do "tensorized" updates of this layer's weight gradients
                %dLdWt = dLdWt + ...
                %    bsxfun(@times, dFdWi, reshape(dLdFi,1,1,o_dim));
                %dLdwt = dLdwt + bsxfun(@times, dFdwi, dLdFi);
                %dLdbt = dLdbt + dLdFi;
            end
            %% Structify the weight gradient tensors
            %dLdW = struct();
            %for i=1:o_dim,
            %    dLdW(i).W = squeeze(dLdWt(:,:,i));
            %    dLdW(i).w = squeeze(dLdwt(:,i));
            %    dLdW(i).b = dLdbt(i);
            %end
            %clear dLdWt dLdwt dLdbt;
